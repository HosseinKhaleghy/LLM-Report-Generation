...
farm-specific advisory report. An automated LLM-as-judge evaluation across
five real dairy farms found the generated reports factually grounded, with
every cited figure traceable to the precomputed analytical data.
\end{abstract}
...
washing, ventilation, and lighting together consume on the order of 200 to
500 kWh per cow per year~\cite{Upton2015}. With electricity prices in Ireland
and across Europe frequently exceeding 0.30 EUR per
kWh~\cite{Eurostat2024}, energy represents a significant and growing share
of operational costs. Despite this, many farm operators rely on a monthly
aggregated electricity bill, which offers limited visibility into where the
load originates, whether consumption is within a healthy range for the herd
size, or when equipment may have malfunctioned.
...
A real hourly electricity CSV file can be uploaded, or a preloaded dataset
representing a real dairy farm~\cite{dairydataset} can be used instead. The coordinator
initiates the pipeline immediately, and each agent's output becomes
inspectable across tabs as it arrives, with the advisory report available
for download on completion. The goal is to show how a multi-agent system decomposes
...
The herd-size estimate and full energy dashboard appear shortly after upload,
as model weights are pre-loaded at application startup. Anomaly detection
runs next and unlocks the Anomalies tab on completion, and the LLM narrative
is then produced section by section, populating the Advisory tab
incrementally so the energy and anomaly tabs can be explored in parallel. A
preloaded farm example ensures the demonstration can begin immediately
without any upload delay.

\subsection{Example Output}

Figure~\ref{fig:screenshot} shows the application running on a real
dairy farm dataset with 8521 hours of recorded consumption. The pipeline
estimated a herd of 164 cows, a total annual
...
An automated LLM-as-judge protocol was applied to reports generated for five
real dairy farms (farms 2, 4, 5, 6, and 7 from the
dataset~\cite{dairydataset}). Each of the eight report sections was scored
on three dimensions: factual grounding (D1, whether every cited figure is
traceable to the farm data), benchmark integrity (D2, whether all cited
thresholds are attributable to the knowledge base), and physical soundness
(D3, whether causal reasoning matches European dairy farm reality). To control
for the self-evaluation bias inherent in using the same model as both
generator and judge, two independent judges were applied. A locally deployed
qwen2.5:14b, drawn from the same family as the narrative generator, returned
mean scores of D1\,=\,5.00, D2\,=\,4.63, D3\,=\,4.85, and overall\,=\,4.70
on a 1--5 scale across the 40 section-level judgements. The same reports,
judged independently by Claude Sonnet 4.6, returned D1\,=\,4.97,
D2\,=\,4.21, D3\,=\,3.75, and overall\,=\,4.00. Factual grounding remained
near perfect under both judges, confirming that pre-computing every cited
figure in the energy analysis agent reliably prevents the LLM from inventing
numbers. The gap in physical-soundness scores (4.85 against 3.75) shows
that the same-family judge was lenient on causal reasoning, supporting the
use of an independent judge in any future evaluation pipeline.
...
A dual-judge LLM-as-judge assessment across five real farms yielded mean
overall scores of 4.70 from a same-family qwen2.5:14b judge and 4.00 from an
independent Claude Sonnet 4.6 judge, with factual grounding near perfect
under both. Future work will validate these findings against domain experts,
explore fine-tuning the narrative agent on expert-reviewed reports, and
extend anomaly classification to equipment-specific fault signatures.
...
Eurostat: Electricity price statistics for household and non-household
consumers in the European Union. European Commission, Brussels (2024).
...
\bibitem{dairydataset}
Vavouris, A., Stankovic, L., Stankovic, V. and Shi, J., 2025. FIELD: a comprehensive FarmIng Electrical LoaD measurements dataset from 30 three-phase dairy farms in Germany. Scientific Data, 12(1), p.1830.
